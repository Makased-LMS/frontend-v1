...
function AddQuestionDialog({ payload, open, onClose }) {
  //TODO: handle data when editing after the quiz created in DB
  ...
  const [questionType, setQuestionType] = useState(() =>
    extractQuestionType(payload?.question.choices)
  );

  const [image, setImage] = useState(payload?.question?.file || null);

  const [currImage, setCurrImage] = useState(
    payload?.question?.currImage || payload?.question?.image?.path || ""
  );
  const imagePreview = useRef(null);

  const [answersCount, setAnswersCount] = useState(
    payload?.question.choices.length || 2
  );

  const [answers, setAnswers] = useState(() =>
    extractChoices(payload?.question.choices)
  );
  const [correctAnswer, setCorrectAnswer] = useState(() =>
    extractCorrectAnswer(payload?.question.choices)
  );
  ...
  const checkAvailabilty = () => {
    let curErrors = {};
    if (points <= 0) curErrors.points = "Points must be greater than 0.";

    if (text.length === 0) curErrors.text = "Question Text is required.";
    ...
    if (
      questionType === 1 &&
      !answers.reduce((pre, ans) => pre && ans.length > 0, true)
    )
  ...
  const handleAddQuestion = () => {
    ...
    if (payload)
      questionsDispatch({
        action: "edit",
        payload: { data: question, index: payload.index },
      });
    else questionsDispatch({ action: "add", payload: { data: [question] } });
  ...
  const handleDeleteAnswer = (index) => {
    ...
    setAnswersCount((c) => c - 1);
    setAnswers((ans) => ans.filter((item, ind) => ind !== index));
  };
  ...
  useEffect(() => {
    if (questionType === 0) {
      ...
      setAnswersCount(payload?.question.choices.length || 2); // MoAs'ad why u put || 2 ??? no need !
      ...
      <Grid container component={DialogContent} sx={{ minHeight: "70vh" }}>
        ...
        >
          ...
          >
            ...
            <Button
              fullWidth
              variant={questionType === 0 ? "outlined" : "text"}
            ...
            <Button
              fullWidth
              variant={questionType === 1 ? "outlined" : "text"}
        ...
        >
          <Grid container>
            ...
            <Grid container flexDirection={"column"} size={2}>
              ...
              <Button component="label" variant="outlined" tabIndex={-1}>
                Upload image
                <VisuallyHiddenInput
                  type="file"
                  accept=".png, .jpg"
                  ...
                  <Grid container alignItems={"center"} spacing={2} size={10}>
                    ...
                    {index === 0 && (
                      <IconButton
                        ...
                        sx={{
                          border: 1,
                          visibility: questionType === 0 ? "hidden" : "unset",
                        }}
